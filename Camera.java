...
public class Camera {
  ...
  public static final double[] DEFAULT_EYE_POSITION = {20.0,12.0,8.0};
  ...
  public void view(GLU glu) {
   ...
   /**
   * Sets the camera to follow the lamp. The lamp is set to be just behind the lamp (10 degree angular offset), and looking in it's direction of travel
   * 
   * Created by Kevin Stanley-Adams 2013
   * 
   * @param glu The GLU object
   * @param angle The current angular position of the lamp
   * @param height The current height the lamp is jumping
   */  
  public void followLamp(GLU glu, double angle, double height) {

      // adjust position and look-at point, up vector is set as the y axis

      glu.gluLookAt(-20*cos(Math.toRadians(-angle+10)),2+height,-20*sin(Math.toRadians(-angle+10)),-20*cos(Math.toRadians(-angle)),2+height,-20*sin(Math.toRadians(-angle)),0,1,0);
